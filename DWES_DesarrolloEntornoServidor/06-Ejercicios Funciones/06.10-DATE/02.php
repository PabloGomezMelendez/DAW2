...
<body>
    <form method="POST" action="01.PHP">
        ...
        <input type="text" name="horas" id="horas" value="" require><br>

        <label for="minutos">Minutos:</label>
        <input type="text" name="minutos" id="minutos" value="" require><br>
    ...
    if (isset($_REQUEST['enviar'])) {
        ...
        if (checkdate($mes, $dia, $anio)) {
            ...
            // $fecha = $mes.'/'.$dia.'/'.$anio;
            $fecha= mktime($horas,$minutos,0,$dia,$mes,$anio );
            $error = false;
        } else {
        ...
        if (!$error) {
            $fehaFinal=date($formato, $fecha);
            echo "<br>Formato de fecha: ". $fehaFinalgtyghtyg;
        }else{
